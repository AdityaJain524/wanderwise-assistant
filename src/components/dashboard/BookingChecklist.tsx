...
const BookingChecklist = () => {
  ...
  const [transportMode, setTransportMode] = useState<'plane' | 'train' | 'bus'>('plane');
  const [expandedId, setExpandedId] = useState<string | null>(null);

  const handleCreate = async () => {
    if (!newTitle.trim()) return;
    await createChecklist(newTitle.trim(), undefined, undefined, isInternational, transportMode);
    setNewTitle('');
          ...
          <DialogContent>
            ...
            <div className="space-y-4 pt-4">
              <div className="space-y-2">
                ...
                <Input
                  placeholder="e.g., Mumbai to Delhi Trip"
                  value={newTitle}
              ...
              <div className="flex items-center space-x-2 pt-2">
                <Switch 
                  ...
                  disabled={transportMode !== 'plane'}
                />
                <Label htmlFor="international-mode" className={cn(transportMode !== 'plane' && "text-muted-foreground")}>
                  International Trip (Requires Visa/Passport)
                </Label>
              </div>
