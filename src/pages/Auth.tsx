...
const Auth = () => {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      } else if (error.message.toLowerCase().includes('rate limit') || error.message.toLowerCase().includes('too many requests') || error.status === 429) {
        toast.error('Too many attempts. Please wait a moment before trying again.');
      } else if (error.status === 400) {
        toast.error('Authentication failed. Please check your credentials or verify your email.');
      } else {
